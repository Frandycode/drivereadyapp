...
const GET_QUESTIONS = gql`
  query GetFlipperQuestions($stateCode: String!, $count: Int!, $chapters: [Int!]) {
...
`
const START_SESSION   = gql`mutation StartFlipperSession($input: StartSessionInput!) { startSession(input: $input) { id } }`
const SUBMIT_ANSWER   = gql`mutation SubmitFlipperAnswer($input: SubmitAnswerInput!) { submitAnswer(input: $input) { isCorrect xpEarned } }`
const COMPLETE_SESSION = gql`mutation CompleteFlipperSession($sessionId: ID!) { completeSession(sessionId: $sessionId) { xpEarned accuracy } }`
...
export function FlipperSession({ config, onExit }: { config: FlipperConfig; onExit: () => void }) {
  ...
  useEffect(() => {
    ...
    startSession({
      variables: { input: { stateCode: config.stateCode, mode: 'flipper', difficulty: config.difficulty, questionCount: questions.length, chapters: chapters ?? [] } },
  ...
  async function handleSubmit(selectedIds: string[]) {
    ...
    const result = await submitAnswer({ variables: { input: { sessionId, questionId: current.id, selectedAnswerIds: selectedIds, hintUsed: false, timeTakenMs: 0 } } })
  ...
  async function handleNext() {
    setRevealed(false); setTimerResetKey((k) => k + 1); setCardKey((k) => k + 1)
  ...
  async function handleSkip() {
    ...
    setTimerResetKey((k) => k + 1); setCardKey((k) => k + 1)
  ...
  if (!current && !showResults) return <div className="min-h-dvh bg-bg flex items-center justify-center"><p className="text-text-secondary text-sm animate-pulse">Loading flipper...</p></div>
  ...
  if (showResults) return (
    <QuizResults
      ...
      difficulty={config.difficulty} deckName={config.chapterTitle ?? 'Flipper'}
  ...
  return (
    <div className="min-h-dvh bg-bg flex flex-col">
      ...
      <div className="flex-1 flex flex-col justify-center py-6">
        <FlipperCard
          key={cardKey} question={current.questionText} explanation={current.explanation}
          answers={current.answers} correctCount={current.correctCount}
          revealed={revealed} onSubmit={handleSubmit}
        />
      </div>
      {revealed && (
